<template>
    ...
    <div>
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <h2 class="mx-auto max-w-2xl text-xl font-bold leading-6 text-gray-700 lg:mx-0 lg:max-w-none">Liste des ventes</h2>
        </div>
              ...
              <table class="w-full text-left p-5">
                <thead class="bg-gray-200 rounded-lg">
                  <tr>
                    ...
                    <th class="">Montant</th>
                    <th class="p-2">Details de la vente</th>
                  </tr>
                </thead>
                <tbody>
                    <tr v-for="transaction in ventes" :key="transaction.id">
                      ...
                      <td class="py-2 pr-6">
                        <div class="text-sm leading-6 font-bold text-gray-700">{{ moment(transaction.date_creation).format('LL') }}</div>
                        <!-- <div class="text-xs text-gray-500">{{ transaction.moyen_paiement }}</div> -->
                      </td>
                      <td class="py-2 pr-6">
                        <div class="text-sm leading-6 font-bold text-gray-700">{{ transaction.montant_total }} F</div>
                        <!-- <div class="text-xs text-gray-500">{{ transaction.moyen_paiement }}</div> -->
                      </td>
                      <td class="py-2 ">
                        <div v-for="produit in transaction.produits_quantite.split(',')" :key="produit" class="flex pl-4">
                          <span class="text-sm font-semibold leading-6 text-indigo-600 hover:text-blue-500">
                            <span class="font-mono">&#8226; {{ produit }}</span>
                            </span>
                        </div>
                      </td>
                    </tr>
...
onMounted(async () => {
    try {
        ...
        throw new Error('Une erreur s\'est produite lors de la récupération des ventes');
        }
    } catch (error) {
        console.error(error);
    }
